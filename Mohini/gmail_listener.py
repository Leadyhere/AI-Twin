...
def send_to_brain(sender, subject, body):
    ...
    try:
        response = requests.post(BRAIN_API_URL, json=payload, timeout=30)
    except requests.RequestException as exc:
        return {
            "clone_draft": "",
            "confidence_score": 0,
            "reasoning": f"Brain API request failed: {exc}",
            "status": "error",
            "error": str(exc)
        }
    ...
    return {
        "clone_draft": "",
        "confidence_score": 0,
        "reasoning": f"Brain API returned HTTP {response.status_code}.",
        "error": response.text,
        "status": "error"
    }
    ...
    try:
        ...
        if not email_ids:
            ...
            for e_id in email_ids:
                ...
                if brain_response.get("error"):
                    print("Error:", brain_response.get("error"))
